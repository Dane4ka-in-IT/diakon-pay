...
public class SMTPService {
    ...
    @Value("${diakonpay.mail.reply:noreply@diakonpay.dev}")
    ...
    protected void sendMail(String to, String subject, String templateName, Map<String, Object> variables) {
        if (!isMailConfigured()) {
            log.warn("Mail not configured (MAIL_USERNAME is unset) — skipping send to {}", to);
            return;
        }
        ...
        try {
            ...
            log.error("Не удалось отправить письмо на адрес {}: {}", to, e.getMessage());
        }
    }
